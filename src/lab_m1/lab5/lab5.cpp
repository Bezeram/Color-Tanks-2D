...
void Lab5::Update(float deltaTimeSeconds)
{
...

    {
        ...
        modelMatrix = glm::rotate(modelMatrix, RADIANS(35.0f), glm::vec3(1, 0, 0));
        RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
    }

    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-10, 2.f, 0));
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-3, 1, 1));
        RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
    }

    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(0, 1, -5));
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    {
        if (window->KeyHold(GLFW_KEY_LEFT_SHIFT))
            multiplier_proj -= 50.f * deltaTime;
        else
            multiplier_proj += 50.f * deltaTime;
	}
    ...
    {
        if (window->KeyHold(GLFW_KEY_LEFT_SHIFT))
            nearPlaneTranslate -= 50.f * deltaTime;
        else
            nearPlaneTranslate += 50.f * deltaTime;
    }
    ...
    {
        if (window->KeyHold(GLFW_KEY_LEFT_SHIFT))
            farPlaneTranslate -= 50.f * deltaTime;
        else
            farPlaneTranslate += 50.f * deltaTime;
    }
}
...
void Lab5::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_O) {
        projectionMatrix = glm::ortho(left * multiplier_proj, right * multiplier_proj, bottom * multiplier_proj, top * multiplier_proj, nearPlane + nearPlaneTranslate, farPlane + farPlaneTranslate);
    }
    ...
    if (key == GLFW_KEY_P) {
        ...
        projectionMatrix = glm::perspective(fov * multiplier_proj, aspectRatio, nearPlane + nearPlaneTranslate, farPlane + farPlaneTranslate);
    }
}
